...
export function usePresence(roomCode, uid, name, role) {
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        // Register the disconnect handler FIRST.
        await onDisconnect(userRef).remove();

        // Then mark the user as online.
        await set(userRef, {
          name,
          role,
          online: true,
    ...
    // Remove presence when the room component is intentionally left.
    return () => {
      console.log("👋 Removing presence for", uid);

      remove(userRef).catch((error) => {
        console.error("❌ Failed to remove presence:", error);
      });
    };
  }, [roomCode, uid, name, role]);
}
...
export function subscribeToPresence(roomCode, onChange) {
  ...
  const unsub = onValue(roomRef, (snapshot) => {
    ...
    const users = Object.entries(data).map(([uid, info]) => ({
      uid,
      ...info,
    }));
